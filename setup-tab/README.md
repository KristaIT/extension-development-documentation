<!-- GPL-3.0; see root LICENSE -->

**Breadcrumbs:** [Home](../index.md) > Setup tab

# Setup tab

## Overview

This section explains how the Setup tab is used to configure and validate your extension.

## Topics

- [Attributes](Attributes.md)
- [Dependencies](Dependencies.md)
- [Authentication](Authentication.md)
- [Trust](Trust.md)
- [Validation and troubleshooting](ValidationAndTroubleshooting.md)
- [Save changes](SaveChanges.md)

## Next steps

- Implement behaviors using [Catalog requests](../development/catalog-requests/README.md).

## Related topics

- [Configuration vs runtime behavior](../concepts/ConfigurationVsRuntimeBehavior.md)
- [Invokers and lifecycle](../concepts/InvokersAndLifecycle.md)





## License

This documentation is licensed under the GNU General Public License v3.0. See [`LICENSE`](../../LICENSE).
