<!-- GPL-3.0; see root LICENSE -->

**Breadcrumbs:** [Home](../index.md) > Concepts

# Concepts

## Overview

Core concepts and terminology used across Krista extensions.

## Topics

- [Key terms](KeyTerms.md)
- [Extension lifecycle](ExtensionLifecycle.md)
- [Invokers and lifecycle](InvokersAndLifecycle.md)
- [Configuration vs runtime behavior](ConfigurationVsRuntimeBehavior.md)
- [Entities and catalog requests](EntitiesAndCatalogRequests.md)



## License

This documentation is licensed under the GNU General Public License v3.0. See [`LICENSE`](../../LICENSE).
